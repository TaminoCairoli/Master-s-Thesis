...
def process_star_file(input_filename, output_filename):
    ...
    in_loop = False
    headers = []
    tilt_index = None  # column index for _rlnAngleTiltPrior

    for line in lines:
        stripped = line.strip()
        
        # Detect the start of the data_particles section.
        if stripped.startswith("data_particles"):
            ...
            # Exit the data_particles block when another data_ block starts.
            in_particles_section = False
            in_loop = False
            headers = []
            tilt_index = None
        ...
        if in_particles_section:
            ...
            # Capture header lines and record the index for _rlnAngleTiltPrior.
            if in_loop and stripped.startswith("_"):
                headers.append(stripped)
                if "_rlnAngleTiltPrior" in stripped:
                    tilt_index = len(headers) - 1
                output_lines.append(line)
                continue

            # Process data lines in the loop block.
            if in_loop and stripped != "":
                tokens = line.split()
                if tilt_index is not None and len(tokens) > tilt_index:
                    try:
                        original_value = float(tokens[tilt_index])
                        new_value = original_value + 90.0
                        tokens[tilt_index] = f"{new_value:.6f}"
                    except ValueError:
...
def main():
    ...
    output_filename = base + "_rotated_Tilt.star"
